...
export default function TasksSummary() {
    ...
    const myTasks = tasks.filter(i => i && user && (i.assignee?._id === user._id || i.assignee === user._id || i.assignee?.id === user._id));
    const overdueTasks = tasks.filter(t => t && t.due_date && new Date(t.due_date) < new Date() && t.status !== 'DONE');
    const inProgressIssues = tasks.filter(i => i && i.status === 'IN_PROGRESS');
