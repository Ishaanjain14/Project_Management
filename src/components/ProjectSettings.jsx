...
export default function ProjectSettings({ project }) {
    ...
    const isTeamLead = project?.team_lead === user?._id || project?.team_lead?._id === user?._id;
    const workspaceMember = currentWorkspace?.members?.find(m => (m.user?._id || m.user) === user?._id);
    const isWorkspaceAdmin = workspaceMember?.role === "OWNER" || workspaceMember?.role === "ADMIN";
    
    const canEdit = isTeamLead || isWorkspaceAdmin;
            ...
            <div className={cardClasses}>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="space-y-2">
                        ...
                        <input value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className={inputClasses} required disabled={!canEdit} />
                    </div>

                    {/* Description */}
                    <div className="space-y-2">
                        <label className={labelClasses}>Description</label>
                        <textarea value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} className={inputClasses + " h-24"} disabled={!canEdit} />
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-2">
                            ...
                            <select value={formData.status} onChange={(e) => setFormData({ ...formData, status: e.target.value })} className={inputClasses} disabled={!canEdit}>
                        ...
                        <div className="space-y-2">
                            ...
                            <select value={formData.priority} onChange={(e) => setFormData({ ...formData, priority: e.target.value })} className={inputClasses} disabled={!canEdit}>
                    ...
                    <div className="space-y-4 grid grid-cols-2 gap-4">
                        <div className="space-y-2">
                            ...
                            <input type="date" value={format(formData.start_date, "yyyy-MM-dd")} onChange={(e) => setFormData({ ...formData, start_date: new Date(e.target.value) })} className={inputClasses} disabled={!canEdit} />
                        </div>
                        <div className="space-y-2">
                            <label className={labelClasses}>End Date</label>
                            <input type="date" value={format(formData.end_date, "yyyy-MM-dd")} onChange={(e) => setFormData({ ...formData, end_date: new Date(e.target.value) })} className={inputClasses} disabled={!canEdit} />
                        </div>
                    </div>
                    ...
                    <div className="space-y-2">
                        ...
                        <input type="range" min="0" max="100" step="5" value={formData.progress} onChange={(e) => setFormData({ ...formData, progress: Number(e.target.value) })} className="w-full accent-blue-500 dark:accent-blue-400" disabled={!canEdit} />
                    </div>
                        ...
                        <div className="space-y-2 mt-2 max-h-32 overflow-y-auto">
                            {project.members.map((member, index) => (
                                <div key={index} className="flex items-center justify-between px-3 py-2 rounded dark:bg-zinc-800 text-sm text-zinc-900 dark:text-zinc-300" >
                                    ...
                                    {project.team_lead === (member.user._id || member.user.id) && <span className="px-2 py-0.5 rounded-xs ring ring-zinc-200 dark:ring-zinc-600">Team Lead</span>}
                                </div>
                            ))}
                        </div>
                    )}
