...
function MyTasksSidebar() {
    ...
    const fetchUserTasks = () => {
        if (!user || !user._id || !currentWorkspace) return;
        const currentWorkspaceTasks = (currentWorkspace.projects || []).flatMap((project) => {
            return (project?.tasks || []).filter((task) => 
                task && (task.assignee?._id === user._id || task.assignee === user._id || task.assignee?.id === user._id)
            );
        });
                    ...
                    <div className="space-y-1">
                        {myTasks.length === 0 ? (
                            ...
                            myTasks.map((task, index) => (
                                <Link key={index} to={`/projects/${task.project || task.projectId}/tasks/${task._id || task.id}`} className="w-full rounded-lg transition-all duration-200 text-gray-700 dark:text-zinc-300 hover:bg-gray-100 dark:hover:bg-zinc-800 hover:text-black dark:hover:text-white" >
